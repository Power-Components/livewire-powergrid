...
@php
    ...
    $filterClasses = Arr::toCssClasses([theme_style($this->theme, 'filterNumber.input')]);
...
@else
    <div
        class="{{ theme_style($this->theme, 'filterNumber.base') }}"
        style="{{ theme_style($this->theme, 'filterNumber.base.1') }}"
    >
        <div>
            <input
                {{ $defaultAttributes['inputStartAttributes'] }}
                @if ($inline) style="{{ theme_style($this->theme, 'filterNumber.input.1') }} {{ data_get($column, 'headerStyle') }}" @endif
                type="text"
                class="{{ $filterClasses }}"
