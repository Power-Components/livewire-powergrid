    ...
    <div class="col-md-12 d-flex table-responsive" style="margin-top: 10px">
        ...
        <span class="d-flex gap-2">
            @foreach($sortArray as $field => $sort)
                ...
                @endphp
                <div wire:key="{{ $tableName }}-multi-sort-{{ $field }}"
                    ...
                    <span class="badge rounded-pill bg-light text-dark">{{ $label }}
                        @if($sort == 'desc')
                            <x-livewire-powergrid::icons.chevron-down width="14" class="ms-1"/>
                        @else
                            <x-livewire-powergrid::icons.chevron-up width="14" class="ms-1"/>
                        @endif
                    </span>
                </div>
