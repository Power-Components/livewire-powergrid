...
<tr class="{{ theme('table.body.tr.wrapper'). ' '.theme('table.body.tr.summarize') }}">
    @if(data_get($setUp, 'detail.showCollapseIcon'))
        <td class="{{ theme('table.body.td.wrapper') }}"></td>
    @endif
    @if($checkbox)
        <td class="{{ theme('table.body.td.wrapper') }}"></td>
    @endif
    @foreach ($this->visibleColumns as $column)
        <td class="{{ theme('table.body.td.summarize.wrapper') . ' '.data_get($column, 'bodyClass') ?? '' }}"
    ...
    @if(isset($actions) && count($actions))
        <th class="{{ theme('table.header.th') .' '. data_get($column, 'headerClass') }}" scope="col"
