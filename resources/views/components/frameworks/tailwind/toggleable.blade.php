    ...
    @if($column->toggleable['enabled'] === true)
        <div class="items-center"
            ...
            <div class="relative rounded-full w-12 h-6 transition duration-200 ease-linear"
                 :class="[toggle_{{ $field }}_{{ $row->id }} === 1 ? 'bg-green-400' : 'bg-gray-400']">
                <label
                ...
                <input type="checkbox"
                       ...
                       @click="saveToggleableInput((toggle_{{ $field }}_{{ $row->id }} === 0 ? toggle_{{ $field }}_{{ $row->id }} = 1 : toggle_{{ $field }}_{{ $row->id }} = 0), {{ $row->id }}, '{{ $field }}')">
            </div>
            @else
