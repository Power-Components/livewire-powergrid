@if(data_get($setUp, 'header.toggleColumns'))
    <div x-data="{open: false}"
        ...
        <div x-show="open"
             ...
             class="mt-2 py-2 w-48 bg-white shadow-xl absolute z-10 dark:bg-slate-500">
            ...
            @foreach($columns as $column)
                ...
                <div wire:click="$emit('pg:toggleColumn-{{ $tableName }}', '{{ $column->field }}')"
                     ...
                     class="@if($column->hidden) opacity-40 @endif cursor-pointer flex justify-start block px-4 py-2 text-slate-800 hover:bg-slate-50 hover:text-black-200 dark:text-slate-200 dark:hover:bg-slate-700">
                    @if(!$column->hidden)
