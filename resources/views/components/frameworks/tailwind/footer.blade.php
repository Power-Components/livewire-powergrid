<div class="flex flex-row w-full flex justify-between pt-3 bg-white overflow-y-auto pl-2 pr-2 pb-1 relative dark:bg-gray-700">
        ...
        <div class="flex flex-row">
            <div class="relative h-10">
                ...
                <select wire:model.lazy="perPage"
                    ...
                    @foreach($perPageValues as $value => $label)
                        <option wire:key="{{ md5($value) }}" value="{{ $label }}">{{ $label }}</option>
                    @endforeach
                </select>
